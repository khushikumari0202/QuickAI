...
const BlogTitles = () => {
  ...
  const blogCategories = [
    'General', 'Technology', 'Business', 'Health', 'Travel', 'Food', 'Lifestyle', 'Education'
  ]
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Generate a catchy and engaging blog title about ${input} in the category of ${selectedCategory}. 
    ...
    gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-bg-border border-gray-200'>
        ...
        <input onChange={(e) => setInput(e.target.value)} value={input} type='text' className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300'
         ...
         <br />
         <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#c341f6] to-[#8e37eb] text-white px-4 py-2 mt-6 text-sm
         rounded-lg cursor-pointer'>
          {loading ? <span className='w-4 h-4 my-1 rounded-full border-2
          border-t-transparent animate-spin'></span> : <Hash className='w-4' />}
          Generate Title
         </button>
      </form>
      {/* right col */}
          ...
          !content ? (
            <div className='flex-1 flex justify-center items-center'>
              <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
                ...
                <p>Start by entering a keyword and clicking on "Generate Title"</p>
              </div>
            </div>
          ) : (
