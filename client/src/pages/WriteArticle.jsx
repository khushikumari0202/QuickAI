...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Write an article about ${input} with a length of around ${selectedLength.text} words. Make sure the article is 
      comprehensive, engaging, and well-structured with an introduction, body, and conclusion. Use clear and concise language, and 
      include relevant examples or data to support key points.`;
    ...
    gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-bg-border border-gray-200'>
        ...
        <input onChange={(e) => setInput(e.target.value)} value={input} type='text' className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300'
         ...
         <br />
         <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#226bff] to-[#65adff] text-white px-4 py-2 mt-6 text-sm
        ...
        {!content ? (
          <div className='flex-1 flex justify-center items-center'>
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              <Edit className='w-5 h-5 text-[#4a7aff]' />
              <p>Start by entering a topic and clicking on "Generate Article"</p>
            </div>
          </div>
        ) : (
