...
const WriteArticle = () => {
  ...
  return (
    ...
    gap-4 text-slate-700'>
      {/* left col */}
      <form onClick={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-bg-border border-gray-200'>
        <div>
        ...
        <input onChange={(e) => setInput(e.target.value)} value={input} type='text' className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300'
         ...
         <br />
         <button className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#226bff] to-[#65adff] text-white px-4 py-2 mt-6 text-sm
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
        ...
        <div className='flex-1 flex justify-center items-center'>
          <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
            <Edit className='w-5 h-5 text-[#4a7aff]' />
            <p>No article generated yet. Start by entering a topic and clicking on "Generate Article"</p>
          </div>
        </div>
